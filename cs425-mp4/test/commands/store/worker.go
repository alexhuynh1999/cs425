...
func main() {
	...
	for {
		...
		case "get":
			...
			client.Get(coordinator_ip, sdfs_name, local_name)

		case "delete":
			// Usage: 'delete [sdfs_file_name]'
			sdfs_name := commandArr[1]
			client.Delete(coordinator_ip, sdfs_name)

		case "ls":
			// Usage: 'ls [sdfs_file_name]'
			sdfs_name := commandArr[1]
			client.ListFiles(coordinator_ip, sdfs_name)

		case "store":
			client.Store(coordinator_ip)
		}
	}
}
